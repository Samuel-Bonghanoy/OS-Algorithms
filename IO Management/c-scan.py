...
class DiskCSCAN:
    ...
    def process_requests(self, requests):
        ...
        # Add end track (track_size - 1) and start track (0) to lists
        if low:
            end_node = RequestNode(self.track_size - 1)
            high = self._insert_sorted(high, end_node)

            start_node = RequestNode(0)
            low = self._insert_sorted(low, start_node)

        # Calculate total head movement
        current_position = self.initial_position
        high_movement = self._calculate_movement(high, current_position)
        low_movement = self._calculate_movement(low, 0)
...
if __name__ == "__main__":
    ...
    disk = DiskCSCAN(initial_position, track_size)
